...
logfile=/esb_archive/log/Remove_ESB.$BASE_NAME_DIR"_"$DATA.log
...
for DIR in `ls -l $BASE_DIR/| grep ^d | awk '{print $9}'`
	...
	find $BASE_DIR/$DIR -mtime +$DEEP -type f -exec rm {} \;	
	rcgzip=$?
	if [ $rcgzip -ne 0 ]
		then
		echo \[${unique_id}\] \[$TIMESTAMPP\] \[WARNING\] "l'eliminazione dei file nella directory $DIR è andata male " >> $logfile
		echo \[${unique_id}\] \[$TIMESTAMPP\] \[WARNING\] "RC:$rcgzip" >> $logfile
		N_OLDER=`expr $N_OLDER + 1`
		export N_OLDER
		continue
		else
		echo \[${unique_id}\] \[INFO\] "l'eliminazione dei file per la directory $DIR e' andata bene"       >> $logfile
...
if [ $N_OLDOK = $N_DIR ] && [ $N_OLDER = 0 ]
	then 
	echo \[${unique_id}\] \[INFO\] "tutte le directory sono state ciclate"       >> $logfile
