...
export logcar=$dirlog/Caricamento_$inputfile.system.log
...
for FILE in `ls $dircenss/* `
do
  CENS=`basename $FILE`
  cp $dircenss/$CENS $RUN
  export rcex=$?
  if [ $rcex -ne 0 ]
    then
  ...
  $dirwsrr/Aclient_Collaudo.sh $CENS 2>/dev/null
