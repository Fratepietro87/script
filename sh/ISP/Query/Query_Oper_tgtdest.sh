...
export AMBIENTE=$4
  ...
  case $AMBIENTE in
           "SYSTEM")
                 ...
                 curl -g -vvvvvv --get "http://ixpwebservices-col.bancaintesa.it:9081/WSRR/7.0/Metadata/XML/GraphQuery" --data-urlencode "query=/WSRR/GenericObject[matches(@CTL_ID_TGT_DES,'$TGTDEST') and matches(@CTL_ID_OPER,'$OPERAZIONE') and matches(@CTL_AMB_LOGICO,'$AMBLOG')  and @namespace='http://www.bancaintesa.com/xmlns/wsrr/catalog']" -o $RUNPATH/Risultati/list_OPERAZIONE.TGTDEST.tmp
